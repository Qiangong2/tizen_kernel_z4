...
int gtd_init(void)
{
	...
	ret = -ENOMEM;
	root = debugfs_create_dir(GT_FOLDER, swap_dentry);
	if (IS_ERR_OR_NULL(root))
		goto fail;

	gt_root = root;

	linker = debugfs_create_dir(GT_LINKER, root);
	if (IS_ERR_OR_NULL(linker)) {
		ret = -ENOMEM;
		goto remove;
	}

	dentry = debugfs_create_file(GT_PATH, GT_DEFAULT_PERMS, linker,
	...
	dentry = debugfs_create_file(GT_FIXUP_ADDR, GT_DEFAULT_PERMS, linker,
				     NULL, &fixup_off_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	...
	dentry = debugfs_create_file(GT_RELOC_ADDR, GT_DEFAULT_PERMS, linker,
				     NULL, &reloc_off_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	...
	by_path = debugfs_create_dir(GT_BY_PATH, root);
	if (IS_ERR_OR_NULL(by_path)) {
		ret = -ENOMEM;
		goto remove;
	}

	dentry = debugfs_create_file(GT_ADD, GT_DEFAULT_PERMS, by_path,
	...
	dentry = debugfs_create_file(GT_DEL, GT_DEFAULT_PERMS, by_path, NULL,
				     &by_path_del_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	...
	by_pid = debugfs_create_dir(GT_BY_PID, root);
	if (IS_ERR_OR_NULL(by_pid)) {
		ret = -ENOMEM;
		goto remove;
	}

	dentry = debugfs_create_file(GT_ADD, GT_DEFAULT_PERMS, by_pid,
	...
	dentry = debugfs_create_file(GT_DEL, GT_DEFAULT_PERMS, by_pid,
	...
	by_id = debugfs_create_dir(GT_BY_ID, root);
	if (IS_ERR_OR_NULL(by_id)) {
		ret = -ENOMEM;
		goto remove;
	}

	dentry = debugfs_create_file(GT_ADD, GT_DEFAULT_PERMS, by_id,
				     NULL, &by_id_add_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	...
	dentry = debugfs_create_file(GT_DEL, GT_DEFAULT_PERMS, by_id,
				     NULL, &by_id_del_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	...
	dentry = debugfs_create_file(GT_DEL_ALL, GT_DEFAULT_PERMS, root,
				     NULL, &del_all_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	...
	dentry = debugfs_create_file(GT_LIST_TARGETS, GT_DEFAULT_PERMS, root,
				     NULL, &target_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	...
	dentry = debugfs_create_file(GT_ENABLE, GT_DEFAULT_PERMS, root,
				     NULL, &enable_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	...
	handler = debugfs_create_dir(GT_HANDLER, root);
	if (IS_ERR_OR_NULL(handler)) {
		ret = -ENOMEM;
		goto remove;
	}

	dentry = debugfs_create_file(GT_PATH, GT_DEFAULT_PERMS, handler,
	...
	dentry = debugfs_create_file(GT_HANDLER_FIXUP_OFF, GT_DEFAULT_PERMS,
				     handler, NULL, &handler_fixup_off_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	}

	dentry = debugfs_create_file(GT_HANDLER_RELOC_OFF, GT_DEFAULT_PERMS,
				     handler, NULL, &handler_reloc_off_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	}

	pthread = debugfs_create_dir(GT_PTHREAD, root);
	if (IS_ERR_OR_NULL(pthread)) {
		ret = -ENOMEM;
		goto remove;
	}

	dentry = debugfs_create_file(GT_PATH, GT_DEFAULT_PERMS, pthread,
	...
	dentry = debugfs_create_file(GT_MINIMAL_INIT, GT_DEFAULT_PERMS,
				     pthread, NULL, &pthread_init_off_fops);
	if (IS_ERR_OR_NULL(dentry)) {
		ret = -ENOMEM;
		goto remove;
	}
