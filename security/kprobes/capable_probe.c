...
extern short g_sec_probe_enabled;
...
static void jp_wake_up_new_task_entry(struct task_struct *p){
	...
	|| !strncmp(process_filter_subsys.command_filter, get_current()->comm, TASK_COMM_LEN)){
		pr_debug("Processs created. comm: %s, pid: %ld\n",
...
static int jp_do_execve_entry(const char *filename,
	...
	|| !strncmp(process_filter_subsys.command_filter, get_current()->comm, TASK_COMM_LEN)){
		pr_debug("Processs execed. comm: %s, comm_execed: %s, pid: %ld\n",
...
#if defined(CONFIG_TIZEN_SEC_KERNEL_ENG) && defined(CONFIG_DYNAMIC_DEBUG)
static struct dentry *g_sec_probe_dir;

static int sec_probe_set(void *data, u64 val)
{
	g_sec_probe_enabled = val;
	return 0;
}

static int sec_probe_get(void *data, u64 *val)
{
	*val  = g_sec_probe_enabled;
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(sec_probe_fops, sec_probe_get, sec_probe_set, "%llu\n");
#endif
...
#if defined(CONFIG_TIZEN_SEC_KERNEL_ENG) && defined(CONFIG_DYNAMIC_DEBUG)
	g_sec_probe_dir = debugfs_create_file("sec_probe", S_IRUGO | S_IWUSR, NULL, NULL, &sec_probe_fops);
	if (!g_sec_probe_dir)
...
#if defined(CONFIG_TIZEN_SEC_KERNEL_ENG) && defined(CONFIG_DYNAMIC_DEBUG)
exit_no_debugfs:
	debugfs_remove(g_sec_probe_dir);
	printk("%s: failed to create sec_probe debugfs entry\n",__FUNCTION__);
#endif
