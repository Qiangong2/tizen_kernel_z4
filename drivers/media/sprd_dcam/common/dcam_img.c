...
LOCAL int sprd_img_setflash(uint32_t flash_mode, void* param)
{
#ifdef CONFIG_CAMERA_MULTIFLASH
	struct dcam_dev *dev = (struct dcam_dev*)param;
	uint32_t flash_index = dev->dcam_cxt.flash_index;
#endif
	...
	case FLASH_TORCH:        /*for torch */
		/*low light */
		#ifdef CONFIG_CAMERA_MULTIFLASH
			sprd_multi_flash_on(flash_index);
		#else
			sprd_flash_on();
		#endif
		break;
	case FLASH_HIGH_LIGHT:
		/*high light */
		#ifdef CONFIG_CAMERA_MULTIFLASH
			sprd_multi_flash_high_light(flash_index);
		#else
	...
	case FLASH_CLOSE:
		/*close the light */
		#ifdef CONFIG_CAMERA_MULTIFLASH
			sprd_multi_flash_close(flash_index);
		#else
			sprd_flash_close();
		#endif
		break;
...
LOCAL int sprd_img_opt_flash(struct dcam_frame *frame, void* param)
{
	...
	if (info->flash_status < FLASH_STATUS_MAX) {
		...
		sprd_img_setflash(info->flash_status, param);
...
#if defined(CONFIG_MACH_J3XLTE) || defined(CONFIG_MACH_J3XNLTE) || defined(CONFIG_MACH_J3X3G) || defined(CONFIG_MACH_J1ACEVELTE) || defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_GTEXSLTE) || defined(CONFIG_MACH_PRIDE_LTE)
...
LOCAL int sprd_img_streampause(struct file *file, uint32_t channel_id, uint32_t reconfig_flag)
{
	...
	/*SI-5786 ioctl SPRD_IMG_IO_PATH_PAUSE invalid parameters crashes kernel*/
...
LOCAL int sprd_img_streamresume(struct file *file, uint32_t channel_id)
{
	...
	/*SI-5788 ioctl SPRD_IMG_IO_PATH_RESUME leak*/
	if(channel_id >= DCAM_PATH_NUM)
		return DCAM_RTN_PARA_ERR;
...
static long sprd_img_k_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#ifdef CONFIG_CAMERA_MULTIFLASH
		dev->dcam_cxt.flash_status = flash_status & 0xff;
		dev->dcam_cxt.flash_index = (flash_status >> 8) & 0xf;
#else
	...
	case SPRD_IMG_IO_PATH_FRM_DECI:
		...
		/*SI-5787 ioctl SPRD_IMG_IO_PATH_FRM_DECI leak*/
		if(parm.channel_id >= DCAM_PATH_NUM) {
			mutex_unlock(&dev->dcam_mutex);
	...
	case SPRD_IMG_IO_SET_SHRINK:
		...
		/*SI-5785 ioctl SPRD_IMG_IO_SET_SHRINK corruption*/
		if (parm.channel_id >= DCAM_PATH_NUM) {
			mutex_unlock(&dev->dcam_mutex);
			ret = DCAM_RTN_PARA_ERR;
			goto exit;
