...
extern unsigned int flash_hw_rev;
void parse_hw_rev(struct device_node *dn)
{
#ifdef CONFIG_OF
	int ret = of_property_read_u32(dn, "hwrev", &flash_hw_rev);

	if (ret) {
		pr_err("%s, HW Rev not entered Default: 0\n", __func__);
	}
#endif
}
#endif
