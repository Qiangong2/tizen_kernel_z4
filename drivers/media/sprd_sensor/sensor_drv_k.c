...
#define SNS_hi544_FW_VERSION_LEN 12
#define SNS_hi544_MODULE_ID_LEN 32

#if defined(CONFIG_MACH_J3XLTE) || defined(CONFIG_MACH_J3XNLTE)|| defined(CONFIG_MACH_J1ACEVELTE)
...
int sensor_k_open(struct inode *node, struct file *file)
{
	...
	/* check current user */
	printk("current camera users: %d Asking pid: %u\n", atomic_read(&p_mod->total_users), current->tgid);
	spin_lock(&g_instance_lock);
	if (atomic_read(&p_mod->total_users) >= 1) {
		spin_unlock(&g_instance_lock);
		if (p_file) {
			vfree(p_file);
			p_file = NULL;
		}
		printk("Error:!!!Total no of camera users Exceeded\n");
		ret = -EBUSY;
		return ret;
	}
...
LOCAL long sensor_k_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SENSOR_IO_IF_CFG:
		...
		case SENSOR_IO_POWER_CFG:
		{
			...
			if (0 == ret) {
#if defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_J1ACEVELTE) || defined(CONFIG_MACH_PRIDE_LTE)
				if(1 == pwr_cfg.is_on) {
		...
		case SENSOR_IO_READ_OTPDATA:
		{
			...
			} else if (type == SENSOR_OTP_PARAM_MODULE_ID) { //For normal binary
				SENSOR_PRINT("SENSOR_OTP_PARAM_MODULE_ID\n");
				char module_id[SNS_hi544_MODULE_ID_LEN] = {0};
				memset(module_id, '\0', sizeof(module_id));
				sensor_get_module_id_otp(module_id);
				copy_to_user(para->buff, module_id, SNS_hi544_MODULE_ID_LEN);
			} else if (type == SENSOR_OTP_PARAM_FW_VERSION) {
				SENSOR_PRINT("SENSOR_OTP_PARAM_FW_VERSION\n");
				char fw_version[SNS_hi544_FW_VERSION_LEN] = {0};
				sensor_get_fw_version_otp(fw_version);
				copy_to_user(para->buff, fw_version, SNS_hi544_FW_VERSION_LEN);
			} else if (type == SENSOR_OTP_PARAM_FW_VERSION_UPDATE) {
				printk("SENSOR_OTP_PARAM_FW_VERSION_UPDATE\n");
				char fw_version[SNS_hi544_FW_VERSION_LEN] = {0};
				copy_from_user(fw_version, para->buff, SNS_hi544_FW_VERSION_LEN);
				sensor_update_fw_version_otp(fw_version);
				printk("fw_version updated");
			} else  {
...
int sensor_k_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&g_instance_lock);
...
#ifndef CONFIG_MACH_PRIDE_LTE
#ifndef CONFIG_MACH_Z2_LTE /*Block unused pin configuration */
...
#if defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE) 		// Set Front Camera GPIO value for DVS
...
#elif defined(CONFIG_MACH_J1MINILTE)  || defined(CONFIG_MACH_Z2_LTE)
...
#define REAR_CAMERA_INFO                 "ISP=INT;CALMEM=Y;READVER=SYSFS;COREVOLT=N;UPGRADE=N;CC=N;OIS=N;"
#define FRONT_CAMERA_INFO                "ISP=INT;CALMEM=N;READVER=SYSFS;COREVOLT=N;UPGRADE=N;CC=N;OIS=N;"
#elif defined(CONFIG_MACH_PRIDE_LTE)
#define REAR_SENSOR_NAME                 "A05PFII01PA A05PFII01PA\n"
#define REAR_FULL_SENSOR_NAME            "A05PFII01PA A05PFII01PA A05PFII01PA\n"
#define FRONT_SENSOR_NAME       	 "S5K5E3YX N\n"
#define FRONT_FULL_SENSOR_NAME  	 "S5K5E3YX N S5K5E3YX\n"
...
{
#if defined(CONFIG_MACH_J3XLTE) || defined(CONFIG_MACH_J3XNLTE) || defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE)
...
static ssize_t Rear_Cam_Module_ID(struct device *dev, struct device_attribute *attr, char *buf)
{
#if defined(CONFIG_MACH_J3XLTE) || defined(CONFIG_MACH_J3XNLTE) || defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE)
	char module_id[32] = {0};
	memset(module_id, '\0', sizeof(module_id));
	sensor_get_module_id_otp(module_id);
	printk("%s : Module id: %s", __func__, module_id);
	return sprintf(buf, "%s\n", module_id);
#else
	SENSOR_PRINT("Rear_Cam_Module_ID\n");
	return sprintf(buf, REAR_SENSOR_NAME);
#endif
}


static ssize_t Rear_Cam_Sensor_Name(struct device *dev, struct device_attribute *attr, char *buf)
{
#if defined(CONFIG_MACH_J3XLTE) || defined(CONFIG_MACH_J3XNLTE) || defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE)
	char sensor_id[32] = {0};
	memset(sensor_id, '\0', sizeof(sensor_id));
	sensor_get_sensor_id_otp(sensor_id);
	printk("%s : sensor id: %s", __func__, sensor_id);
	return sprintf(buf, "%s\n", sensor_id);
#else
	SENSOR_PRINT("Rear_Cam_Sensor_Name\n");
	return sprintf(buf, REAR_SENSOR_NAME);
#endif
}
...
{
#if defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE)
...
static DEVICE_ATTR(rear_cam_module_id, S_IRUGO | S_IXOTH, Rear_Cam_Module_ID, NULL); // Read(User, Group, Other), Execute(Other)
static DEVICE_ATTR(rear_cam_sensor_id, S_IRUGO | S_IXOTH, Rear_Cam_Sensor_Name, NULL); // Read(User, Group, Other), Execute(Other)
...
int __init sensor_k_init(void)
{
	...
	err = device_create_file(dev_t_rear, &dev_attr_rear_cam_module_id);
	if (err) {
		SENSOR_PRINT("Failed to create device file(%s)!\n", dev_attr_rear_caminfo.attr.name);
		goto err_make_rear_cam_module_id;
	}

	err = device_create_file(dev_t_rear, &dev_attr_rear_cam_sensor_id);
	if (err) {
		SENSOR_PRINT("Failed to create device file(%s)!\n", dev_attr_rear_caminfo.attr.name);
		goto err_make_rear_cam_sensor_id;
	}
...
#endif
	device_remove_file(dev_t_rear, &dev_attr_rear_cam_sensor_id);
err_make_rear_cam_sensor_id:
	device_remove_file(dev_t_rear, &dev_attr_rear_cam_module_id);
err_make_rear_cam_module_id:
