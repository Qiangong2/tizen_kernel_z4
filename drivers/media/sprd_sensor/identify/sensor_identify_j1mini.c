...
void Sr030pc50_mipi_yuv_PowerOn_j1minilte(struct sensor_file_tag *sensor_file, int is_poweron)
{
...
void gc0310_mipi_yuv_poweron_j1minilte(struct sensor_file_tag *sensor_file, int is_poweron)
{
	uint32_t *fd_handle = (uint32_t*)sensor_file;
...
sensor_hwinfto_t front_cameras[] = {
	{
		...
		.poweron	= Sr030pc50_mipi_yuv_PowerOn_j1minilte,
#else
	...
	{
#if defined(CONFIG_MACH_J1MINILTE) || defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE)
		.poweron	= gc0310_mipi_yuv_poweron_j1minilte,
#else
