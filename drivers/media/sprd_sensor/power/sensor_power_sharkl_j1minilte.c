...
#define S5K5E3YX_SLAVE_ADDR	       0x10
...
static int sensor_sr544_poweron(uint32_t *fd_handle, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	...
	sensor_k_set_voltage_cammot(fd_handle, SENSOR_VDD_2800MV); /* AF_2.8V */
	mdelay(5); /*min 2ms in spec */

	sensor_k_set_voltage_iovdd(fd_handle, SENSOR_VDD_1800MV); /* IO/5M_HOST 1.8V */
	mdelay(1);

	sensor_k_set_voltage_avdd(fd_handle, SENSOR_VDD_2800MV); /* 5M_AVDD 2.8V */
	mdelay(1);

	sensor_k_set_voltage_dvdd(fd_handle, SENSOR_VDD_1200MV); /* 5M_DVDD 1.2V */
	mdelay(1);

	sensor_k_set_pd_level(fd_handle, 1); /* 5M_XSHUTDOWN */
	udelay(500);

	sensor_k_set_mclk(fd_handle, 24); /* MCLK (24MHz기준) To 5M */
	mdelay(15); /* min 10ms in spec */
...
static int sensor_sr544_poweroff(uint32_t *fd_handle, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	...
	sensor_k_set_pd_level(fd_handle, 0); /* 5M_XSHUTDOWN */
	mdelay(1);

	sensor_k_set_voltage_dvdd(fd_handle, SENSOR_VDD_CLOSED); /* 5M_DVDD 1.2V */
	mdelay(1);

	sensor_k_set_voltage_avdd(fd_handle, SENSOR_VDD_CLOSED); /* 5M_AVDD 2.8V */
	mdelay(1);

	sensor_k_set_voltage_iovdd(fd_handle, SENSOR_VDD_CLOSED); /* IO/5M_HOST 1.8V */
	mdelay(5); /* min 2ms in spec */

	sensor_k_set_voltage_cammot(fd_handle, SENSOR_VDD_CLOSED); /* AF_2.8V */
	udelay(10);
...
{
	int ret = 0;
	int tmp_id = ((struct sensor_file_tag *)fd_handle)->sensor_id;
	...
	sensor_k_sensor_sel(fd_handle, tmp_id);
	return ret;
}
...
static int sensor_s5k5e3yx_poweron(uint32_t *fd_handle, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	...
	sensor_k_set_rst_level(fd_handle, 1); /* reset 1 */
	udelay(50);
	sensor_k_set_pd_level(fd_handle, 1); /* power down 1*/
	udelay(250);
...
static int sensor_s5k5e3yx_poweroff(uint32_t *fd_handle, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	...
	sensor_k_set_pd_level(fd_handle, 0); /* power down 0 */
	sensor_k_set_rst_level(fd_handle, 0); /* reset 0 */
...
int sensor_power_on(uint32_t *fd_handle, uint32_t sensor_id, struct sensor_power *dev0, struct sensor_power *dev1, struct sensor_power *dev2)
{
	...
	if (SENSOR_DEV_0 == sensor_id) {
		...
		} else if (S5K5E3YX_SLAVE_ADDR == i2c_slave_addr) {
