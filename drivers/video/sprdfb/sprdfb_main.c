			...
			unsigned long arg)
{
	...
	case SPRD_FB_METADATA_SET:
		result = copy_from_user(&metadata, argp, sizeof(metadata));
		if (result) {
			pr_err("%s: copy_from_user failed", __func__);
			return result;
		}

		if (NULL != dev->ctrl->set_metadata)
			result = dev->ctrl->set_metadata(dev, &metadata);
		break;
	default:
				...
				struct device_attribute *attr, char *buf)
{
	...
	pr_debug("%s:layer[%d]osd[%pK]img[%pK]\n", __func__,
	...
	pos += sprintf(buf+pos, "layer[%s], osd[0x%x], img[0x%x], ",
			layer_type,
			(unsigned int)fb_dev->overlay_data.osd_buffer,
			(unsigned int)fb_dev->overlay_data.img_buffer);
