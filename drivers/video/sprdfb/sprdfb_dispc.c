...
#if ((defined CONFIG_FB_SCX35L) || (defined CONFIG_FB_SCX15) || (defined CONFIG_FB_SCX30G))
#define SPRDFB_NR_BRIGHTNESS           (0x02<<16)//(0x03<<16)// 9-bits
#define SPRDFB_NR_CONTRAST             (0x12A<<0) //10-bits
#define SPRDFB_NR_OFFSET_U             (0x81<<16)//8-bits
#define SPRDFB_NR_SATURATION_U         (0x123<<0)//(0x12A<<0)//10-bits
#define SPRDFB_NR_OFFSET_V             (0x82<<16)//8-bits
#define SPRDFB_NR_SATURATION_V         (0x123<<0)//(0x12A<<0)//10-bits

#define SPRDFB_WD_BRIGHTNESS           (0x00<<16)//(0x03<<16)// 9-bits
#define SPRDFB_WD_CONTRAST             (0x100<<0) //10-bits
#define SPRDFB_WD_OFFSET_U             (0x80<<16)//8-bits
#define SPRDFB_WD_SATURATION_U         (0x100<<0)//(0x12A<<0)//10-bits
#define SPRDFB_WD_OFFSET_V             (0x80<<16)//8-bits
#define SPRDFB_WD_SATURATION_V         (0x100<<0)//(0x12A<<0)//10-bits
#else
...
static int overlay_set_csc(struct sprdfb_device *dev, uint8_t csc_range)
{
	int ret = 0;

	pr_info("%s:csc_range[%d->%d]\n", __func__, dev->csc_range, csc_range);
...
#else
	if (csc_range == dev->csc_range)
		goto out;

	if (csc_range == CSC_RANGE_LIMITED) {
		dispc_write(SPRDFB_NR_BRIGHTNESS|SPRDFB_NR_CONTRAST, DISPC_Y2R_Y_PARAM);
		dispc_write(SPRDFB_NR_OFFSET_U|SPRDFB_NR_SATURATION_U, DISPC_Y2R_U_PARAM);
		dispc_write(SPRDFB_NR_OFFSET_V|SPRDFB_NR_SATURATION_V, DISPC_Y2R_V_PARAM);
		dev->csc_range = CSC_RANGE_LIMITED;
	} else if (csc_range == CSC_RANGE_FULL) {
		dispc_write(SPRDFB_WD_BRIGHTNESS|SPRDFB_WD_CONTRAST, DISPC_Y2R_Y_PARAM);
		dispc_write(SPRDFB_WD_OFFSET_U|SPRDFB_WD_SATURATION_U, DISPC_Y2R_U_PARAM);
		dispc_write(SPRDFB_WD_OFFSET_V|SPRDFB_WD_SATURATION_V, DISPC_Y2R_V_PARAM);
		dev->csc_range = CSC_RANGE_FULL;
	} else
		ret = -EINVAL;
out:
#endif
	...
	overlay_rect *rect, overlay_endian *endian, bool rb_switch)
{
	...
	if(dispc_is_yuv_format(type)) {
		...
		if (csc_range != CSC_RANGE_FULL)
			csc_range = CSC_RANGE_LIMITED;
		overlay_set_csc(dev, csc_range);
	}
		...
		overlay_rect *rect, overlay_endian *endian, bool rb_switch)
{
	...
	reg_value = (y_endian<<8)|(type << 4)|(1<<2)|BIT(17);
...
static int32_t sprdfb_set_metadata(struct sprdfb_device *dev, struct overlay_metadata *metadata)
{
	...
	switch (metadata->op) {
	case metadata_op_csc:
		overlay_set_csc(dev, metadata->data.csc_range);
		break;
	default:
...
out:
	pr_info("%s:op[%d]ret[%d]\n", __func__, metadata->op, ret);
	return ret;
}
...
static int32_t sprdfb_dispc_display_overlay(struct sprdfb_device *dev, struct overlay_display* setting)
{
	...
	if (setting->layer_index == SPRD_LAYER_BOTH)
		usleep_range(300, 310);
#endif
#endif

	if (SPRD_LAYER_IMG == setting->layer_index) {
		size = &dispc_ctx.overlay_img_info.size;
		...
		size = &dispc_ctx.overlay_osd_info.size;
	...
	if ((dev->panel_if_type == SPRDFB_PANEL_IF_DPI) && dispc_ctx.vsync_waiter)
		dispc_sync(dev);
	...
	if ((setting->layer_index & SPRD_LAYER_IMG) && img_buffer) {
		...
		if (dispc_is_yuv_format(dispc_ctx.overlay_img_info.data_type)) {
			uint32_t buf_size = size->hsize * size->vsize;

			dispc_write((uint32_t)(img_buffer + buf_size), DISPC_IMG_UV_BASE_ADDR);

			if (dispc_ctx.overlay_img_info.data_type == SPRD_DATA_FORMAT_YUV422_3P)
				dispc_write((uint32_t)(img_buffer + buf_size * 3 / 2),
						DISPC_IMG_V_BASE_ADDR);
			else if (dispc_ctx.overlay_img_info.data_type == SPRD_DATA_FORMAT_YUV420_3P)
				dispc_write((uint32_t)(img_buffer + buf_size * 5 / 4),
						DISPC_IMG_V_BASE_ADDR);
		}
	}
...
struct display_ctrl sprdfb_dispc_ctrl = {
	...
	.set_metadata = sprdfb_set_metadata,
};
