	...
	if (on) {
		if (pin->type == EXT_PIN_REGULATOR)	{
			...
			if (unlikely(rc)) {
				...
				pr_info("[LCD] regulator(%s) is enabled\n",
						pin->name);
			}
		} else {
		...
		if (pin->type == EXT_PIN_REGULATOR) {
			...
			if (unlikely(rc)) {
				...
				pr_info("[LCD] regulator(%s) is disabled\n",
						pin->name);
			}
		} else {
			if (!gpio_is_valid(pin->gpio)) {
...
static void gen_panel_set_power(struct lcd *lcd, int on)
{
	...
	if ((on && (lcd->power == FB_BLANK_UNBLANK))
			|| (!on && (lcd->power == FB_BLANK_POWERDOWN))) {
...
void gen_panel_set_status(struct lcd *lcd, int status)
{
	...
	int skip_init = (status == GEN_PANEL_SLEEP_OUT);
...
static DEVICE_ATTR(lcd_power, S_IRUGO | S_IXOTH, lcd_power_show, NULL);
...
#endif
err_lcd_power:
	device_remove_file(lcd->dev, &dev_attr_lcd_power);
err_window_type:
