...
static int rt5033_mask_all_irqs(struct i2c_client *iic)
{
	...
	for (i = 0; i < ARRAY_SIZE(rt5033_irq_ctrl_regs); i++) {
		...
		if (rc < 0) {
			dev_info( chip->dev,"Error : can't write reg[0x%x] = 0x%x\n",
...
int rt5033_init_irq(rt5033_mfd_chip_t *chip)
{
	...
	if (ret < 0) {
		dev_info( chip->dev, "%s : Can't mask all irqs(%d)\n", __func__, ret);
	...
	if (ret < 0) {
		dev_info( chip->dev,"%s : Request GPIO %d failed\n",
	...
	if (ret < 0) {
		dev_info( chip->dev,"Set GPIO direction to input : failed\n");
	...
	ret = request_threaded_irq(chip->irq, NULL, rt5033_irq_handler,
				   ...
				   "rt5033", chip);
	if (ret < 0) {
		dev_info( chip->dev,"%s : Failed : request IRQ (%d)\n", __func__, ret);
	...
	if (ret < 0) {
		dev_info( chip->dev,"%s : enable_irq_wake(%d) failed for (%d)\n",
