			    ...
			    const struct i2c_device_id *id)
{
	...
	dev_info(&i2c->dev, "%s : RT5033 MFD Driver start probe\n", __func__);
	if (of_node) {
		pdata = devm_kzalloc(&i2c->dev, sizeof(*pdata), GFP_KERNEL);
		if (!pdata) {
			dev_info(&i2c->dev, "Failed to allocate memory\n");
			ret = -ENOMEM;
			goto err_dt_nomem;
		}
	...
	dev_info(&i2c->dev, "%s:%s pdata->irq_base = %d\n",
	...
	if (pdata->irq_base < 0) {
		dev_info(&i2c->dev, "%s:%s irq_alloc_descs Fail! ret(%d)\n",
		       "rt5033-mfd", __func__, pdata->irq_base);
		ret = -EINVAL;
		goto irq_base_err;
	} else {
		chip->irq_base = pdata->irq_base;
		dev_info(&i2c->dev, "%s:%s irq_base = %d\n",
	...
	dev_info(&i2c->dev, "%s : Manual Reset Data = 0x%x", __func__, data);
...
#endif
	if (ret < 0) {
		dev_info(&chip->dev, "Failed : add FlashLED devices");
...
#endif
	if (ret < 0) {
		dev_info(chip->dev, "Failed : add charger devices\n");
		goto err_add_chg_devs;
	}
#endif /*CONFIG_CHARGER_RT5033*/

	dev_info(&i2c->dev,"%s : RT5033 MFD Driver Fin probe\n", __func__);
	return ret;
...
static int rt5033_mfd_remove(struct i2c_client *i2c)
{
	...
	printk("%s : RT5033 MFD Driver remove\n", __func__);
	mfd_remove_devices(chip->dev);
	wake_lock_destroy(&(chip->irq_wake_lock));
	mutex_destroy(&chip->suspend_flag_lock);
	mutex_destroy(&chip->regulator_lock);
...
static int __init rt5033_mfd_i2c_init(void)
{
	...
	if (ret != 0)
		printk("%s : Failed to register RT5033 MFD I2C driver\n",
			__func__);
