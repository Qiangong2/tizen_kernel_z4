...
static const struct id_desc id_to_cable_type_mapping[] = {
	...
	{			/* 11111, 31 */
	 ...
	 .cable_type_with_vbus = MUIC_SM5504_CABLE_TYPE_USB,
...
static ssize_t rustproof_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	RTINFO("SM5504 rustproof_show\n");

	/* is_rustproof is false, UART enabled.*/
	return sprintf(buf, "%d\n", enable_muic_uart);
}

static ssize_t rustproof_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t size)
{
	RTINFO("SM5504 rustproof_store\n");
...
static DEVICE_ATTR(rustproof, S_IRUGO | S_IWUSR ,
				rustproof_show, rustproof_store);
#endif
...
#if defined(CONFIG_MUIC_SUPPORT_RUSTPROOF)
	{
		...
		if ((is_usrbin == 1) &&
			((type == JIG_UART_BOOT_OFF) ||
				(type == JIG_UART_BOOT_ON))) {
			if (val == 1) {
				sm5504_reg_write(chip,
					SM5504_REG_MANUAL_SW1, 0x00);
				//auto switching -> manual switching
				sm5504_clr_bits(chip, SM5504_REG_CONTROL,
							1 << 2);
				RTINFO("Open JIG UART PATH in case of battery on USR binary\n");
			} else { // no batt
				sm5504_reg_write(chip, SM5504_REG_MANUAL_SW1,
							0x6C);
				//auto switching -> manual switching
				sm5504_clr_bits(chip, SM5504_REG_CONTROL,
						1 << 2);
...
/* for printint out registers -- end*/
	...
	RTINFO("Status : cable type = %d, id_adc= 0X%X\n"
	       "vbus = %d, accessory = %d\n"
	       "ocp = %d, ovp = %d, otp = %d,\n"
	...
	if (chip->curr_status.vbus_status != chip->prev_status.vbus_status) {
		...
		if (param.vbus_status == 0) {
			if (!psy || !psy->set_property) {
				RTINFO("%s: fail to get battery psy\n",
					__func__);
			} else {
				RTINFO("%s, vbus 0, send cable type as battery\n",
					__func__);
				value.intval = POWER_SUPPLY_TYPE_BATTERY;
				psy->set_property(psy, POWER_SUPPLY_PROP_ONLINE, &value);
			}
		} else if (param.vbus_status == 1 && chip->curr_status.id_adc == 0x16) {
			/*
			 * when vbus goes from 0 to 1, if cable id 0x16 is
			 * connected with device then cable change
			 * notification is not coming. Send the notification
			 * to battery
			 */
			RTINFO("%s, cable type = 0x%X\n",
				__func__, chip->curr_status.cable_type);
			cable_change_callback(chip->curr_status.cable_type);
		}
	} else {
		RTINFO("no vbus \n");
...
#ifdef CONFIG_MACH_PRIDE_LTE
	/* In Pride, no support in BL to distinguish ENG/USR */
	is_usrbin = !get_sec_debug_level();
	RTINFO("Kernel Binary: %s\n", is_usrbin ? "USR" : "ENG");
#else
	{/*check usr binary*/
		...
		if (val_sm5504_reg13 == 0x0C) {
			is_usrbin = 1;
			val_sm5504_reg2 = sm5504_reg_read(chip,
							SM5504_REG_CONTROL);
			if (!(val_sm5504_reg2 & 0x04)) {
				//manual switching -> auto switching
				sm5504_set_bits(chip, SM5504_REG_CONTROL,
						1 << 2);
}
		}
			...
			struct device_attribute *attr, char *buf)
{
	...
	if (current_status->cable_type == MUIC_SM5504_CABLE_TYPE_JIG_UART_OFF) {
		...
		return sprintf(buf, "%d\n", adc_fail);
#elif !defined(CONFIG_USB_SPRD_SWITCH)
	if (chip_g)
		adc_reg_value = sm5504_reg_read(chip_g, chip_g->adc_reg_addr);
	if (adc_reg_value < 0) {
		RTINFO("adc_show no detect\n");
		return sprintf(buf, "%d\n", adc_fail);
	}
	RTERR("%s line %d : adc_value:0X%X\n", __FUNCTION__,__LINE__,adc_reg_value);
...
static ssize_t jig_cable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	RTINFO("SM5504 jig_cable_show\n");
...
static DEVICE_ATTR(jig_cable, S_IRUGO, jig_cable_show, NULL);
