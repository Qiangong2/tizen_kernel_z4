...
static int rt5033_pmic_reset_by_regulator(struct regulator_dev *rdev);
...
static int rt5033_regulator_enable(struct regulator_dev *rdev)
{
	...
	if (rt5033_regulator_get_status(rdev) == REGULATOR_STATUS_ERROR) {
		pr_info("Regulator failed, try to reset it\n");
		rt5033_pmic_reset_by_regulator(rdev);
		return -EINVAL;
	}
	return ret;
}
...
/* WARNING : need to modify by Samsung */
#define GPIO_SCL	127
#define GPIO_SDA	128
#define GPIO_MRSTB	0x25

static int rt5033_pmic_reset_by_regulator(struct regulator_dev *rdev)
{
	struct rt5033_regulator_info *info = rdev_get_drvdata(rdev);
	int rev_id;
	rev_id = rt5033_reg_read(info->i2c, 0x03) & 0x0f;


	/* Enable internal clock, otherwise RESET circuit can't work */
	rt5033_set_bits(info->i2c, 0x6b, 0x01);
	/* Turn on safe LDO, otherwise RESET circuit will not work */
	rt5033_set_bits(info->i2c, RT5033_REGULATOR_REG_OUTPUT_EN,
			RT5033_REGULATOR_EN_MASK_LDO_SAFE);
	if (rev_id >= 6) {
		rt5033_reg_write(info->i2c, 0x47, 0x88);
		/* Optional... */
		gpio_request(GPIO_SCL, "gpio_scl"); // maybe no need to request
		gpio_request(GPIO_SDA, "gpio_sda"); // maybe no need to request

		gpio_direction_output(GPIO_SCL, 0);
		gpio_direction_output(GPIO_SDA, 0);
		msleep(1000); /* Actuall 750ms... but different model might get different condition */
	} else {
		rt5033_reg_write(info->i2c, 0x47, 0x08);
		gpio_request(GPIO_MRSTB, "gpio_mrstb");
		gpio_direction_output(GPIO_MRSTB, 0);
		gpio_free(GPIO_MRSTB);
		msleep(3500);
	}
