...
int save_data_to_file(char *path, void *data, int size) {
	...
	written_size = vfs_write(fp, (const char __user *)data
							, size, &fp->f_pos);
...
int get_phy_addr_value(unsigned int phy_addr) {

	void __iomem *phy_addr_iomem;
	int value;

	phy_addr_iomem = ioremap(phy_addr, 4);
	if (phy_addr_iomem == NULL)
		goto err;

	value = readl(phy_addr_iomem);

	iounmap(phy_addr_iomem);
...
int n_square(int base_num, int expo)
{
	int i, value;
...
int get_phy_addr_bit_value(int addr, int upper, int lower) {

	int value, bit_mask, bit_value;


	value = get_phy_addr_value(addr);
	pr_info("%X=0x%X\n", addr, value);

	bit_mask = (n_square(2, upper + 1)) - (n_square(2, lower) - 1);

	pr_info("upper=%X lower=%X bit_mask=0x%X\n",upper, lower, bit_mask);

	bit_value = (value & bit_mask) >> lower;
	pr_info("bit_value=0x%X\n", bit_value);
