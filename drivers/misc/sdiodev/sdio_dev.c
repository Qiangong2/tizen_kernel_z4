...
/*
 * 0: not complete
 * 1: complete and timeout
 * 2: complete and success
 */
...
int set_marlin_wakeup(uint32 chn,uint32 user_id)
{
	...
	int screen_status = 1;
	int bt_state ;
	SDIOTRAN_DEBUG("entry,user_id = %d",user_id);
	...
	if(0 == sleep_para.gpio_opt_tag)
	{
		...
		if((0x1 == user_id) ||(0x3 == user_id))
		{
			...
			if (ret == 0){
				...
				SDIOTRAN_ERR("marlin chn %d ack timeout, user_id-%d, sdio_cnt %d,fdl_cnt %d,sch_cnt %d!!!",
						chn,user_id,marlin_sdio_req_cnt,fdl_sdio_req_cnt,sch_sdio_req_cnt);
...
int set_marlin_sleep(uint32 chn,uint32 user_id)
{
	...
	if( atomic_read(&gpioreq_need_pulldown) )//if(sleep_para.gpioreq_need_pulldown)
	{
		...
		SDIOTRAN_DEBUG("gpio%d-0,user_id-%d\n", sdio_data->wake_out, user_id);
	}
#endif
	return 0;
}
...
int sdiodev_readchn_init(uint32 chn,void* callback,bool with_para)
{
	...
	if(with_para == 1)
	{
		...
		SDIOTRAN_DEBUG("sdio_tran_handle, chn = %d, callback = %p",
				sdio_tran_handle[chn].chn,sdio_tran_handle[chn].tran_callback_para);
	}
	else
	{
		sdio_tran_handle[chn].tran_callback = callback;		
		SDIOTRAN_DEBUG("sdio_tran_handle, chn = %d, callback = %p",
				sdio_tran_handle[chn].chn,sdio_tran_handle[chn].tran_callback);
	}	
...
static irqreturn_t marlinwake_irq_handler(int irq, void * para)
{
	...
	if ((gpio_wake_status) && atomic_read(&gpioreq_need_pulldown))//if(sleep_para.gpioreq_need_pulldown)
	{
	//judge set_marlin_cmplete for handling the case "marlin_ack response slowly from cp"
		if (atomic_read(&(set_marlin_cmplete)) == 1)
		{
...
static int marlin_sdio_suspend(struct device *dev)
{
	...
	else if (atomic_read(&marlin_wake_done) == 1)
