...
#define EN_MEASURE_INPUT_CURRENT 0
#define EN_PMIC_BUGFIX	1
...
#define EOC_NORMAL_DEBOUNCE_CNT    10
#define EOC_RECHG_DEBOUNCE_CNT         3
...
#endif /* CONFIG_FLED_RT5033 */
#if EN_MEASURE_INPUT_CURRENT
	struct delayed_work mivr_counter_reset_work;
	struct wake_lock measure_wake_lock;
	struct mutex mivr_lock;
	bool mivr_irq_enabled;
	int mivr_irq;
	int mivr_count;
	int report_input_current;
	int report_charging_current;
#endif /* EN_MEASURE_INPUT_CURRENT */
} rt5033_charger_data_t;

#if EN_MEASURE_INPUT_CURRENT
static void increase_charging_current(rt5033_charger_data_t* charger);
static void decrease_charging_current(rt5033_charger_data_t* charger);
static void disable_mivr_irq(rt5033_charger_data_t* charger, bool sync);
static void enable_mivr_irq(rt5033_charger_data_t* charger);
#endif

#if defined(CONFIG_MACH_PRIDE_LTE)
...
static void rt5033_read_regs(struct i2c_client *i2c, char *str)
{
	...
	for (i = 0; i < ARRAY_SIZE(rt5033_reg_map); i++) {
		...
		sprintf(str + strlen(str), "reg0x%02x = 0x%02x, ", rt5033_reg_map[i], data);
	}
}
...
/* VBATT = 4150 --> MIVR setting = 4500 */
static int rt5033_est_best_mivr_setting(int vbatt)
{
	int mivr;
	mivr = (vbatt + 50) + 300;
	if (mivr < 4200)
		...
		mivr = 4600;
	pr_info("Best MIVR level = %d mV, vbatt = %d\n", mivr, vbatt);
	return (mivr - 4100) / 100;
}
		...
		int onoff)
{
	...
	if ((onoff > 0) && (prev_charging_status == false)) {
		...
		rt5033_clr_bits(iic, 0x41, 1 << 6);
		msleep(1);
#endif 		/* EN_PMIC_BUGFIX */
#ifdef CONFIG_FLED_RT5033
		if (charger->fled_info == NULL)
			charger->fled_info = rt_fled_get_info_by_name(NULL);
		if (charger->fled_info)
...
#if EN_MEASURE_INPUT_CURRENT
		mutex_lock(&charger->mivr_lock);
		charger->mivr_count = 0;
		charger->aicl_on = false;
		mutex_unlock(&charger->mivr_lock);
		psy_do_property("sec-fuelgauge", get,
				POWER_SUPPLY_PROP_VOLTAGE_NOW,
				val);
		vbatt = val.intval;
		mivr_setting = rt5033_est_best_mivr_setting(vbatt);
		rt5033_assign_bits(iic, RT5033_CHG_CTRL4, RT5033_MIVR_MASK,
			mivr_setting << RT5033_MIVR_SHIFT);
		enable_mivr_irq(charger);
		mutex_unlock(&charger->io_lock);
		decrease_charging_current(charger);
		mutex_lock(&charger->io_lock);
		wake_lock_timeout(&charger->measure_wake_lock,
			msecs_to_jiffies(500));
		queue_delayed_work(charger->wq,
			&charger->mivr_counter_reset_work,
			msecs_to_jiffies(350));
#endif /* EN_MEASURE_INPUT_CURRENT */

		/* Reset EOC loop, and make it re-detect */
		charger->eoc_cnt = 0;
		rt5033_enable_charging_termination(iic, 1);
		...
		if (charger->rev_id >= 6)
			rt5033_assign_bits(iic, 0x47, 0xFF, 0x80);

#if EN_PMIC_BUGFIX
		msleep(30);
		/* Turn on SafeLDO */
		rt5033_set_bits(iic, 0x41, 1 << 6);
		msleep(1);
		...
		swelling_mode = val.intval;
#endif
#if defined(CONFIG_MACH_PRIDE_LTE)
		psy_do_property("battery", get,
				POWER_SUPPLY_PROP_CHARGE_COUNTER, val);
		abs_timer_exp = val.intval;
		...
		charger->eoc_cnt = 0;
		/* Set EOC level = disabled */
		rt5033_assign_bits(iic, RT5033_CHG_CTRL4, RT5033_IEOC_MASK, 0);
#ifdef CONFIG_FLED_RT5033
		if (charger->fled_info == NULL)
			charger->fled_info = rt_fled_get_info_by_name(NULL);
		if (charger->fled_info)
		...
		if (val.intval == POWER_SUPPLY_STATUS_NOT_CHARGING ||
			...
			swelling_mode || abs_timer_exp || chg_on) {
			 ...
			 */
			if (charger->rev_id >= 6)
				rt5033_set_bits(iic, 0x47, 0x8F);
		}
#if EN_MEASURE_INPUT_CURRENT
		disable_mivr_irq(charger, true);
		cancel_delayed_work(&charger->mivr_counter_reset_work);
#endif /* EN_MEASURE_INPUT_CURRENT */
	} else {
...
static void rt5033_set_tdeg_eoc(struct rt5033_charger_data *charger,
		int tdeg_eoc)
{
	struct i2c_client *i2c = charger->rt5033->i2c_client;
	int data;
	...
	mutex_lock(&charger->io_lock);
	rt5033_assign_bits(i2c, RT5033_CHG_CTRL2, RT5033_TDEG_EOC_MASK,
			   tdeg_eoc << RT5033_TDEG_EOC_SHIFT);
	mutex_unlock(&charger->io_lock);
...
#ifdef CONFIG_SIOP_CHARGING_LIMIT_CURRENT
...
#if (ENABLE_MIVR || EN_MEASURE_INPUT_CURRENT)
...
static void rt5033_configure_charger(struct rt5033_charger_data *charger)
{
...
#endif /*DISABLE_MIVR or EN_MEASURE_INPUT_CURRENT*/
	psy_do_property("battery", get,
			POWER_SUPPLY_PROP_CHARGE_NOW, val);
	input_current_limit = charger->pdata->charging_current
	...
	/* set TDEG_EOC to max value 11:256us */
	rt5033_set_tdeg_eoc(charger, 0x03);
	...
	pr_info("%s : termination current (%dmA)\n",
		__func__, eoc);
#if EN_MEASURE_INPUT_CURRENT
	charger->report_charging_current = charger->charging_current;
	charger->report_input_current = input_current_limit;
#endif /* EN_MEASURE_INPUT_CURRENT */
...
static bool rt5033_chg_init(struct rt5033_charger_data *charger)
{
	...
	rt5033_chg_fled_init(i2c);
	/* Disable Timer function (Charging timeout fault) */
	rt5033_clr_bits(i2c, RT5033_CHG_CTRL3, RT5033_TIMEREN_MASK);
	/* Enable TE */
	rt5033_enable_charging_termination(i2c, 1);

	/* Enable High-GM */
	rt5033_set_bits(i2c, 0x07, 0xC0);

	/* Set input current limit => 500mA */
	rt5033_set_input_current_limit(charger, 500);

	/* EMI improvement , let reg0x18 bit2~5 be 1100*/
	rt5033_assign_bits(i2c, 0x18, 0x3C, 0x30);
...
#endif
	/* Set Reg0x07[5] = 0 -> enable bad adaptor detection */
	rt5033_clr_bits(i2c, RT5033_EOC_CTRL, (1 << 5));
	/* Set Reg0x19[3] = 1 -> Disable SafeLDO OVP */
	rt5033_set_bits(i2c, 0x19, 1 << 3);
...
#if 0 //def CONFIG_USB_HOST_NOTIFY
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		#if EN_MEASURE_INPUT_CURRENT
		/* We use "POWER_SUPPLY_PROP_CURRENT_AVG" to
		 * report measured input current */
		val->intval = charger->report_input_current;
		#endif
		break;
...
#if EN_MEASURE_INPUT_CURRENT
			val->intval = MINVAL(chg_curr,
				charger->report_charging_current);
#else
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (charger->cable_type == POWER_SUPPLY_TYPE_BATTERY ||
			...
			/* Enable charger */
			if (charger->full_charged)
				charger->eoc_debounce_max = EOC_RECHG_DEBOUNCE_CNT;
			else
				charger->eoc_debounce_max = EOC_NORMAL_DEBOUNCE_CNT;
			rt5033_configure_charger(charger);
			rt5033_set_input_current_limit(charger, 500);
			charger->charging_current = 500;
			rt5033_set_charging_current(charger, 150, 0); //charging with 500mA
			rt5033_enable_charger_switch(charger, 1);

			msleep(100);

			rt5033_configure_charger(charger);
			rt5033_enable_charger_switch(charger, 1); //charging with normal charge current
		}
#if EN_TEST_READ
		msleep(100);
		rt5033_test_read(charger->rt5033->i2c_client);
...
{
	struct rt5033_charger_data *charger =
		container_of(psy, struct rt5033_charger_data, psy_otg);
...
{
	struct i2c_client* i2c = charger->rt5033->i2c_client;
	if (charger->report_charging_current >= 700) {
		pr_info("increase charging current, previous = %d, new = %d\n",
					charger->report_charging_current,
					charger->report_charging_current + 100);
		charger->report_charging_current += 100;
		__rt5033_set_fast_charging_current(i2c,
			charger->report_charging_current);
	} else if (charger->report_charging_current == 500) {
		/* AICR was set to 500mA
		 * restore AICR setting */
		rt5033_set_input_current_limit(charger, charger->report_input_current);
		charger->report_charging_current = 700;
	}
}
static void decrease_charging_current(rt5033_charger_data_t* charger)
{
	struct i2c_client* i2c = charger->rt5033->i2c_client;
	if (charger->report_charging_current >= 800) {
		charger->report_charging_current -= 100;
		__rt5033_set_fast_charging_current(i2c,
			charger->report_charging_current);
	} else {
		/* Set AICR to 500mA
		 * ICC keeps 700mA */
		charger->report_charging_current = 500;
		rt5033_set_input_current_limit(charger, 500);
	}
}
...
{
	mutex_lock(&charger->mivr_lock);
	if (charger->mivr_irq_enabled) {
		if (sync)
			disable_irq(charger->mivr_irq);
		else
			disable_irq_nosync(charger->mivr_irq);
		charger->mivr_irq_enabled = false;
	}
	mutex_unlock(&charger->mivr_lock);
}
static void enable_mivr_irq(rt5033_charger_data_t* charger)
{
	mutex_lock(&charger->mivr_lock);
	if (!charger->mivr_irq_enabled) {
		enable_irq(charger->mivr_irq);
		charger->mivr_irq_enabled = true;
	}
	mutex_unlock(&charger->mivr_lock);
}


static int est_input_current(int vbatt, int cc)
{
	int dV = vbatt - 3200;
	return (1782 * cc + (440 * dV * cc) / 1024 - 72 * dV) / 2048 - 74;
}


#define WARN_IF(cond, format, args...) \
	if (cond) \
	printk(KERN_WARNING \
		"rt5033_charger:%s() line-%d: " format, \
		__func__, __LINE__, ## args)


static void rt5033chg_reset_mivr_counter(
				struct work_struct *work)
{
	rt5033_charger_data_t *charger =
			container_of(to_delayed_work(work),
					rt5033_charger_data_t,
					mivr_counter_reset_work);
	int vbatt, cc;
	int estimated_input_current;
	union power_supply_propval psy_val;
	mutex_lock(&charger->mivr_lock);
	pr_info("MIVR counter reset...\r\n");
	charger->mivr_count = 0;
	mutex_unlock(&charger->mivr_lock);
	disable_mivr_irq(charger, true);
	increase_charging_current(charger);
	if (charger->report_charging_current >= 700) {
		cc = charger->report_charging_current;
		wake_lock_timeout(&charger->measure_wake_lock,
		...
		psy_do_property("sec-fuelgauge", get,
				POWER_SUPPLY_PROP_VOLTAGE_NOW,
				psy_val);
		vbatt = psy_val.intval;
		estimated_input_current = est_input_current(vbatt, cc - 50) * 40 / 42;
		pr_info("cc = %d, vbatt = %d, estimated input current = %d\r\n"
									, cc , vbatt, estimated_input_current);
		if (charger->report_input_current > estimated_input_current)
			charger->report_input_current = estimated_input_current;
	} else
		charger->report_input_current = 500;
	if (charger->report_input_current < SLOW_CHARGING_CURRENT_STANDARD)
		charger->aicl_on = true;
	pr_info("Measured input current = %d mA\r\n",
				charger->report_input_current);
#if ENABLE_MIVR
	rt5033_set_mivr_level(charger);
#else
	rt5033_assign_bits(charger->rt5033->i2c_client,
			   RT5033_CHG_CTRL4, RT5033_MIVR_MASK, RT5033_MIVR_DISABLE << RT5033_MIVR_SHIFT);
#endif
}

#define MIVR_DEGLITCH 5
#define MIVR_SLEEP_TIME 10


static irqreturn_t rt5033_chg_mivr_irq_handler(int irq, void *data)
{
	rt5033_charger_data_t *charger = data;
	bool measurement_done = false;

	pr_info("mivr count = %d\r\n", charger->mivr_count);
	cancel_delayed_work(&charger->mivr_counter_reset_work);
	mutex_lock(&charger->mivr_lock);
	if (charger->mivr_count < MIVR_DEGLITCH) {
			charger->mivr_count++;
			mutex_unlock(&charger->mivr_lock);
			msleep(MIVR_SLEEP_TIME);
			goto mivr_irq_exit;
	}
	/* Reset counter */
	charger->mivr_count = 0;
	mutex_unlock(&charger->mivr_lock);
	if (charger->report_charging_current <= 500) {
		/* AICR had already set to 500mA, but MIVR is still triggered */
		disable_mivr_irq(charger, false);
		charger->report_charging_current = 400;
		charger->report_input_current = 400;
		if (charger->report_input_current <
	...
	pr_info("report_input_current = %d\r\n",
				charger->report_input_current);
mivr_irq_exit:
	if (!measurement_done) {
		wake_lock_timeout(&charger->measure_wake_lock,
			msecs_to_jiffies(MIVR_SLEEP_TIME * MIVR_DEGLITCH * 3));
		queue_delayed_work(charger->wq,
			&charger->mivr_counter_reset_work,
			msecs_to_jiffies(MIVR_SLEEP_TIME * MIVR_DEGLITCH * 2));
	}
    return IRQ_HANDLED;
}
...
static irqreturn_t rt5033_chg_ieoc_irq_handler(int irq, void *data)
{
	...
	pr_info("%s : EOC CNT = %d / %d\n", __func__,
		info->eoc_cnt, info->eoc_debounce_max);
	if (info->eoc_cnt >= info->eoc_debounce_max) {
		...
		info->eoc_cnt = 0;
		/* Once battery was full charged,
		 * we will change the EOC debounce times
		 */
		info->eoc_debounce_max = EOC_RECHG_DEBOUNCE_CNT;
...
#endif
#if EN_MEASURE_INPUT_CURRENT
	irq_name = rt5033_get_irq_name_by_index(RT5033_CHMIVRI_IRQ);
	irq = platform_get_irq_byname(pdev, irq_name);
	info->mivr_irq = irq;
	info->mivr_irq_enabled = true;
#endif
...
static int rt5033_charger_probe(struct platform_device *pdev)
{
	...
	charger->psy_otg.get_property   = sec_otg_get_property;
	charger->psy_otg.set_property   = sec_otg_set_property;
	charger->psy_otg.properties             = sec_otg_props;
	charger->psy_otg.num_properties = ARRAY_SIZE(sec_otg_props);
...
#if EN_MEASURE_INPUT_CURRENT
	INIT_DELAYED_WORK(&charger->mivr_counter_reset_work,
		rt5033chg_reset_mivr_counter);
	wake_lock_init(&charger->measure_wake_lock,
			WAKE_LOCK_SUSPEND,
		       "rt5033chg_wakelock");
#endif /* EN_MEASURE_INPUT_CURRENT */
...
err_reg_irq:
#if EN_MEASURE_INPUT_CURRENT
	wake_lock_destroy(&charger->measure_wake_lock);
#endif /* EN_MEASURE_INPUT_CURRENT */
...
#if EN_MEASURE_INPUT_CURRENT
	mutex_destroy(&charger->mivr_lock);
#endif /* EN_MEASURE_INPUT_CURRENT */
