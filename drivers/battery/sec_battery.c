...
static bool fb_client_reg;
static bool lcd_state; /* 1 - ON, 0 - OFF */
...
#endif
	POWER_SUPPLY_PROP_BATTERY_CAPACITY,
};
...
static unsigned long pause_time;
static unsigned long pause_start_time;

static unsigned long sec_bat_get_current_time(void)
{
...
static int sec_bat_fb_notifier_cb(struct notifier_block *self,
				unsigned long event, void *data)
{
	...
	case FB_EVENT_BLANK:
		...
		case FB_BLANK_UNBLANK:
			lcd_state = 1; //LCD ON
			pause_start_time = sec_bat_get_current_time();
			break;
		default:
			return 0;
		}
	...
	case FB_EARLY_EVENT_BLANK:
		...
		case FB_BLANK_POWERDOWN:
			lcd_state = 0; //LCD OFF
			current_time = sec_bat_get_current_time();
			if (current_time >= pause_start_time)
				pause_time += current_time - pause_start_time;
			else
				pause_time += 0xFFFFFFFF - pause_start_time
						+ current_time;
			pr_info("%s : charge pause time : %lu",
					__func__, pause_time);
			break;
		default:
			return 0;
...
static void sec_bat_register_fb_notifier(void)
{
	struct timespec ts;

	pr_info("%s\n", __func__);

	if (fb_client_reg == false) {
		memset(&fb_notif, 0, sizeof(fb_notif));

		fb_notif.notifier_call = sec_bat_fb_notifier_cb;
		pause_time = 0;
		lcd_state = backlight_enable;
		pause_start_time = sec_bat_get_current_time();
...
static void sec_bat_unregister_fb_notifier(void)
{
				...
				bool enable)
{
	...
	if (enable) {
		sec_bat_register_fb_notifier();
		val.intval = battery->cable_type;
		battery->safety_timer_exp = false;
		...
		sec_bat_unregister_fb_notifier();
...
static bool sec_bat_check_recharge(struct sec_battery_info *battery)
{
	...
	if (battery->swelling_mode && battery->temperature >= battery->pdata->swelling_high_temp_block) {
	...
	} else if (battery->swelling_mode && battery->temperature <= battery->pdata->swelling_high_temp_block) {
		if ((battery->charging_mode == SEC_BATTERY_CHARGING_NONE) &&
			(battery->retail_mode == false)) {
			if ((battery->pdata->recharge_condition_type &
						SEC_BATTERY_RECHARGE_CONDITION_VCELL) &&
...
#endif
	/*
	 * If charging has not stopped and safety timer has experied then do
	 * not allow recharge untill charger removed and inserted again
	 */
	if (battery->safety_timer_exp) {
		dev_info(battery->dev,
			"%s: safety timer expired, block recharge\n",
			 __func__);
		return false;
	}
...
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	if (battery->swelling_mode) {
		...
		if ((temperature <= battery->pdata->swelling_high_temp_recov) &&
			...
			battery->swelling_mode = false;
			/*
			 * Once issue : IEOC interrupt came in RT5033
			 * pmic, pmic changed the state from charging to full
			 * but before battery could send stop charge, swelling
			 * charge recover happend and pmic state machine went
			 * wrong so because of this charge was stopping before
			 * IEOC IRQ generged. So, to maintain the state machine
			 * of RT5033 pmic, stop the charge before starting.
			 */
			sec_bat_set_charge(battery, false);
			battery->charging_mode = SEC_BATTERY_CHARGING_1ST;
				...
				struct sec_battery_info *battery)
{
	...
	unsigned long charging_total_time; /* set for TA & USB */
	struct timespec ts;
#if defined(ANDROID_ALARM_ACTIVATED)
	ktime_t current_time;
	...
	if (lcd_state == 1) {
		/*
		 * LCD is on, update pause time
		 */
		if (ts.tv_sec >= pause_start_time)
			pause_time += ts.tv_sec - pause_start_time;
		else
			pause_time += 0xFFFFFFFF - pause_start_time
				+ ts.tv_sec;
		pause_start_time = ts.tv_sec;

		dev_info(battery->dev,
			"%s: Charge pause time : %lu secs\n", __func__, pause_time);
	}
	...
	case POWER_SUPPLY_STATUS_CHARGING:
		if (battery->cable_type == POWER_SUPPLY_TYPE_MAINS)
			charging_total_time = battery->pdata->charging_total_time; /* TA */
		else
			charging_total_time = 2 * battery->pdata->charging_total_time; /* USB */
		charging_total_time += pause_time;
		...
		if ((battery->pdata->full_condition_type &
			...
			(charging_time > charging_total_time)) {
			dev_info(battery->dev,
				"%s: Charging Timer Expired\n", __func__);
#if 0 //no full charge UI in case of safety timer expire
			if (battery->pdata->full_condition_type &
				SEC_BATTERY_FULL_CONDITION_NOTIMEFULL) {
				if (battery->capacity >= 100)
					sec_bat_set_charging_status(battery,
				...
				/*
				 * if battery is not full then do not set
				 * battery status as full.
				 */
				//sec_bat_set_charging_status(battery,
				//			    POWER_SUPPLY_STATUS_FULL);
			}
...
#endif
	} else if ((battery->capacity == 100) || (battery->retail_mode == true)){
		battery->charging_mode = SEC_BATTERY_CHARGING_NONE;
		battery->is_recharging = false;
		sec_bat_set_charge(battery, false);
					...
					const char *buf, size_t count)
{
	...
	case BATT_TEMP:
		if (sscanf(buf, "%d\n", &x) == 1) {
			if (x == 1) {
				...
				battery->temp_recover_cnt = 0;
				battery->health = POWER_SUPPLY_HEALTH_GOOD;
				dev_info(battery->dev, "%s: TEMP_CHECK_NONE by sysfs\n", __func__);
			} else {
				battery->pdata->temp_check_type = SEC_BATTERY_TEMP_CHECK_TEMP;
				dev_info(battery->dev, "%s: TEMP_CHECK_TEMP by sysfs\n", __func__);
			}
		}
		break;
	case BATT_TEMP_ADC:
	...
	case CHG_RETAIL_MODE:
		if (sscanf(buf, "%d\n", &x) == 1) {
			...
			if (x) {
				...
				battery->pdata->full_condition_type = 12;
				battery->pdata->full_check_type = SEC_BATTERY_FULLCHARGED_SOC;
				battery->pdata->full_condition_soc = 70;
				battery->pdata->full_condition_vcell = 3800;
				battery->pdata->recharge_condition_type = SEC_BATTERY_RECHARGE_CONDITION_SOC;
				battery->pdata->recharge_condition_soc = 60;
				for (i=0; i<POWER_SUPPLY_TYPE_MAX; i++) {
					battery->pdata->charging_current[i].full_check_current_1st = 70;
				}
			} else {
				battery->retail_mode = false;
				battery->pdata->full_condition_type = 13;
				battery->pdata->full_check_type = SEC_BATTERY_FULLCHARGED_CHGPSY;
				battery->pdata->full_condition_soc = 100;
				battery->pdata->full_condition_vcell = 4238;
				battery->pdata->recharge_condition_type = SEC_BATTERY_RECHARGE_CONDITION_VCELL;
				for (i=0; i<POWER_SUPPLY_TYPE_MAX; i++) {
					battery->pdata->charging_current[i].full_check_current_1st = 100;
				}
			}
			ret = count;
		}
	...
	case CHG_ON:
		if (sscanf(buf, "%d\n", &x) == 1) {
			...
			dev_info(battery->dev, "%s: Charging status(%d), charging mode (%d)\n",
				__func__, battery->status, battery->charging_mode);
			power_supply_changed(&battery->psy_bat);
			ret = count;
		}
		break;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		val->intval = battery->safety_timer_exp;
		break;

	case POWER_SUPPLY_PROP_CHARGE_ENABLED:
		val->intval = battery->chg_on;
		break;
#endif

	case POWER_SUPPLY_PROP_BATTERY_CAPACITY :
		val->intval = battery->pdata->batt_capacity;
		break;
	default:
		return -EINVAL;
	}
...
#if defined(CONFIG_MACH_PRIDE_LTE)
static void sec_battery_detect_isr_work(struct work_struct *work)
{
	union power_supply_propval ps_val;

	ps_val.intval = gpio_get_value(EIC_BATDET);
	pr_info("%s: Battery Present = %d\n",
			__func__, ps_val.intval);

	if (!ps_val.intval)
		psy_do_property("battery", set, POWER_SUPPLY_PROP_PRESENT, ps_val);
}

static irqreturn_t sec_batt_detect_irq(int irq, void *dev_id)
{
	struct sec_battery_info *battery = dev_id;
	union power_supply_propval ps_val;

	ps_val.intval = gpio_get_value(EIC_BATDET);
	irq_set_irq_type(irq,
		(ps_val.intval ? IRQF_TRIGGER_LOW : IRQF_TRIGGER_HIGH));

	if (battery)
		schedule_delayed_work(&battery->bat_detect, 0);
...
#if defined(CONFIG_MACH_PRIDE_LTE)
	INIT_DELAYED_WORK(&battery->bat_detect, sec_battery_detect_isr_work);
#endif
...
#endif
#if defined(CONFIG_MACH_PRIDE_LTE)
	gpio_request(EIC_BATDET, "batt_detect_irq");
	gpio_direction_input(EIC_BATDET);
	battery->irq = gpio_to_irq(EIC_BATDET);

	ret = request_irq(battery->irq, sec_batt_detect_irq,
				IRQF_SHARED | IRQF_TRIGGER_LOW,
				"batt_detect_irq", battery);
#endif
...
#if defined(CONFIG_MACH_PRIDE_LTE)
	cancel_delayed_work_sync(&battery->bat_detect);
#endif
	alarm_cancel(&battery->event_termination_alarm);
