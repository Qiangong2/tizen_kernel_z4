...
#if defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE)
#define SIOP_LEVEL_100_CHG_CURRENT	700
#define SIOP_LEVEL_70_CHG_CURRENT	475
#define SIOP_LEVEL_50_CHG_CURRENT	350
#define SIOP_LEVEL_30_CHG_CURRENT	200
#define SIOP_LEVEL_100			100
#define SIOP_LEVEL_70			70
#define SIOP_LEVEL_50			50
#define SIOP_LEVEL_30			30
#endif
...
static int SM5701_get_charging_health(struct SM5701_charger_data *charger)
{
	...
	pr_info("%s : Health, charger->cable_type : 0x%02x\n", __func__, charger->cable_type);
...
static u8 SM5701_otg_control(struct SM5701_charger_data *charger, int enable)
{
	...
	if (led_state_charger == LED_DISABLE)
		mask = 0x02; //enable only OTG
	else
		mask = 0x03; //enable Flash & OTG
		...
		if ((charger->status == POWER_SUPPLY_STATUS_CHARGING) ||
			...
			if ((value.intval == POWER_SUPPLY_HEALTH_UNSPEC_FAILURE) ||
				...
				pr_info("%s : vbus current limit (%dmA)\n",
					...
					[charger->cable_type].input_current_limit);
#if defined(CONFIG_BATTERY_JXX_LTE) && !defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE)
			...
			/* Adjust the charging current as per SIOP level */
#if defined(CONFIG_MACH_Z2_LTE) || defined(CONFIG_MACH_PRIDE_LTE)
			if (charger->siop_level == SIOP_LEVEL_100)
				charger->charging_current = SIOP_LEVEL_100_CHG_CURRENT;
			else if (charger->siop_level == SIOP_LEVEL_70)
				charger->charging_current = SIOP_LEVEL_70_CHG_CURRENT;
			else if (charger->siop_level == SIOP_LEVEL_50)
				charger->charging_current = SIOP_LEVEL_50_CHG_CURRENT;
			else if (charger->siop_level == SIOP_LEVEL_30)
				charger->charging_current = SIOP_LEVEL_30_CHG_CURRENT;
#endif
...
#elif defined(CONFIG_MACH_J13G) || defined(CONFIG_MACH_YOUNG2VE3G) || \
