...
#define DRIVER_VERSION  "3.10.0_ps_only_20150603_2"
...
#define STK_STATE_EN_OFS_MSK            0x08
...
#define CALIBRATION_FILE_PATH	"/csa/sensor/prox_cal"
#endif
enum {
	OFF = 0,
	ON,
...
static int32_t stk3013_enable_ps(struct device *dev, uint8_t enable, uint8_t validate_reg)
{
	...
	if (enable)
		w_state_reg |= (STK_STATE_EN_PS_MASK | STK_STATE_EN_WAIT_MASK
				| STK_STATE_EN_OFS_MSK | STK_STATE_EN_ASO_MASK);
	...
	if (enable) {
		...
		if (hrtimer_active(&ps_data->prox_timer)) {
...
static int proximity_store_calibration(struct device *dev, bool do_calib)
{
	...
	if (do_calib) {
		...
		if (ret < 0) {
			...
			pr_info("fail offset calibration = %d\n",
					offset_data);
			ps_data->ps_offset = ps_data->ps_default_offset;
		}
	} else {
...
static void stk_work_func(struct work_struct *work)
{
	...
	if (org_flag_reg & STK_FLG_PSINT_MASK) {
		...
		pr_info("%s: FACTORY: near/far=%d, ps code = %d\n", __func__,
				near_far_state, read_value);
#else
		pr_info("%s: near/far=%d, ps code = %d\n", __func__,
				near_far_state, read_value);
		if ((near_far_state == 0 && read_value >= ps_data->ps_thd_h)
			|| (near_far_state == 1
			&& read_value <= ps_data->ps_thd_l))
#endif
		{
...
#else
	ret = request_any_context_irq(irq, stk_oss_irq_handler,
			IRQF_TRIGGER_LOW|IRQF_NO_SUSPEND, "proximity_int", ps_data);
#endif
	if (ret < 0) {
...
static int stk3013_regulator_onoff(struct device *dev, bool onoff)
{
	...
	if (!ps_data->vio) {
		...
		ps_data->vio = devm_regulator_get(dev, "RT5033LDO1");
		if (IS_ERR(ps_data->vio)) {
			pr_err("%s, cannot get vio\n", __func__);
			return -ENOMEM;
		}
		regulator_set_voltage(ps_data->vio, 3000000, 3000000);
	}
	ret = gpio_request(ps_data->pdata->power_en, "prox_en");
	if (ret) {
	...
	if (onoff) {
		ret = gpio_direction_output(ps_data->pdata->power_en, 1);
		if (ret) {
			pr_err("%s: unable to set_direction [%d]\n",
					__func__, ps_data->pdata->power_en);
			goto err_gpio_set_fail;
		}
		...
		ret = gpio_direction_output(ps_data->pdata->power_en, 0);
		if (ret) {
			pr_err("%s: unable to set_direction [%d]\n",
					__func__, ps_data->pdata->power_en);
			goto err_gpio_set_fail;
	...
	gpio_free(ps_data->pdata->power_en);
	return 0;
			...
			struct stk3013_platform_data *pdata)
{
	...
	pdata->power_en = of_get_named_gpio_flags(np, "stk,en-gpio", 0,
		&pdata->int_flags);
	if (pdata->power_en < 0) {
		dev_err(dev, "Unable to read en-gpio\n");
		return pdata->power_en;
	}
	...
	rc = of_property_read_u32(np, "stk,ps-off", &temp_val);
	if (!rc)
		pdata->ps_default_offset = (u8)temp_val;
	else {
		dev_err(dev, "Unable to read offset-reg\n");
		return rc;
	}
...
#endif /* !CONFIG_OF */
#ifdef PROXIMITY_CALIBRATION
static int check_calibration_offset(struct stk3013_data *pdata)
{
	struct file *cal_filp = NULL;
	mm_segment_t old_fs;
	...
	if (IS_ERR(cal_filp)) {
		...
		if (ret != -ENOENT)
			pr_err("%s, Can't open calibration file\n",
				__func__);
		set_fs(old_fs);
		pdata->ps_offset = pdata->ps_default_offset;
		pr_err("%s, Can't open calibration file 2, %d\n",
				__func__, ret);
		return ret;
	}

	if (file_offset_data < pdata->ps_cal_skip_adc)
		goto exit;

	if (file_offset_data != pdata->ps_offset)
		pdata->ps_offset = file_offset_data;
	if (pdata->ps_offset != pdata->ps_default_offset) {
		stk3013_set_ps_thd_h(pdata, pdata->ps_cancel_thd_h);
		stk3013_set_ps_thd_l(pdata, pdata->ps_cancel_thd_l);
	}
exit:
	pr_err("%s: file_offset = %d, ps_offset = %d, default_offset = %d\n",
		__func__, file_offset_data, pdata->ps_offset,
		pdata->ps_default_offset);
...
static int stk3013_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = stk3013_regulator_onoff(&client->dev, ON);
	if (ret < 0) {
		pr_err("%s: unable to get regulator\n",  __func__);
		goto err_als_input_allocate;
	}
...
static int stk3013_remove(struct i2c_client *client)
{
	...
	free_irq(ps_data->irq, ps_data);
	gpio_free(ps_data->int_pin);
#if defined(CONFIG_SENSORS_STK3013_REGULATOR)
	if (ps_data->vdd) {
		pr_info("%s VDD put\n", __func__);
