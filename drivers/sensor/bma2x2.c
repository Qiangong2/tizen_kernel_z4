...
#define TIME_HI_HI_MASK                (0xFFFF000000000000)
#define TIME_HI_HI_SHIFT       (48)
#define TIME_HI_LO_MASK                (0x0000FFFF00000000)
#define TIME_HI_LO_SHIFT       (32)
#define TIME_LO_HI_MASK                (0x00000000FFFF0000)
#define TIME_LO_HI_SHIFT       (16)
#define TIME_LO_LO_MASK                (0x000000000000FFFF)
#define RAW_LOG_TIME           15      /* 15 sec */
					...
					struct bma2x2_data *data)
{
	...
	unsigned char bma2x2_sensor_type_count;
...
static int bma2x2_get_slope_first(struct i2c_client *client,
...
static int bma2x2_get_slope_sign(struct i2c_client *client,
		unsigned char *intstatus)
{
	int comres;
	unsigned char data;
...
static void bma2x2_work_func(struct work_struct *work)
{
	...
	u64 delay_ns = atomic_read(&bma2x2->delay) * 1000000ULL;
	u64 shift_timestamp;
	u64 timestamp;
	int ret;
	...
	if (((timestamp_new - bma2x2->old_timestamp)
		...
		for (timestamp = bma2x2->old_timestamp + delay_ns;
			timestamp < timestamp_new - shift_timestamp;
				timestamp += delay_ns) {
			...
			input_report_rel(bma2x2->input, (REL_Z + 1),
				((timestamp & TIME_HI_HI_MASK) >> TIME_HI_HI_SHIFT));
			input_report_rel(bma2x2->input, (REL_Z + 2),
				((timestamp & TIME_HI_LO_MASK) >> TIME_HI_LO_SHIFT));
			input_report_rel(bma2x2->input, (REL_Z + 3),
				((timestamp & TIME_LO_HI_MASK) >> TIME_LO_HI_SHIFT));
			input_report_rel(bma2x2->input, (REL_Z + 4),
				(timestamp & TIME_LO_LO_MASK));

			input_sync(bma2x2->input);
			bma2x2->old_timestamp = timestamp;
		}
	}
	...
	input_report_rel(bma2x2->input, (REL_Z + 1),
		((timestamp_new & TIME_HI_HI_MASK) >> TIME_HI_HI_SHIFT));
	input_report_rel(bma2x2->input, (REL_Z + 2),
		((timestamp_new & TIME_HI_LO_MASK) >> TIME_HI_LO_SHIFT));
	input_report_rel(bma2x2->input, (REL_Z + 3),
		((timestamp_new & TIME_LO_HI_MASK) >> TIME_LO_HI_SHIFT));
	input_report_rel(bma2x2->input, (REL_Z + 4),
		(timestamp_new & TIME_LO_LO_MASK));
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", atomic_read(&bma2x2->delay)*1000000);
				...
				const char *buf, size_t count)
{
	...
	if (bw >= 0x08) {
		error = bma2x2_set_bandwidth(bma2x2->bma2x2_client, bw);
		if (error < 0) {
					...
					const char *buf, size_t count)
{
	...
	if (bma2x2->n_irq) {
		if (!value) {
			disable_irq_wake(bma2x2->n_irq);
			disable_irq(bma2x2->n_irq);
		} else {
			enable_irq(bma2x2->n_irq);
			enable_irq_wake(bma2x2->n_irq);
		}
	}
...
#ifdef BMA2X2_SELF_TEST
static DEVICE_ATTR(selftest, 0660, bma2x2_selftest_show,
		bma2x2_selftest_store);
#endif
		...
		const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		SENSOR_ERR("failed to set gpio %d as input (%d)\n",
			data->acc_int1, err);
		goto exit_gpio_direction_input;
	}

	data->n_irq = gpio_to_irq(data->acc_int1);
	err = request_irq(data->n_irq, bma2x2_irq_handler,
			IRQF_TRIGGER_RISING | IRQF_ONESHOT | IRQF_NO_SUSPEND,
			"bma2x2", data);
	if (err)
		SENSOR_ERR("could not request irq\n");

	disable_irq(data->n_irq);
	...
	input_set_capability(dev, EV_REL, (REL_Z + 1));
	input_set_capability(dev, EV_REL, (REL_Z + 2));
	input_set_capability(dev, EV_REL, (REL_Z + 3));
	input_set_capability(dev, EV_REL, (REL_Z + 4));
...
exit_alloc_input_device:
	free_irq(data->n_irq, data);
exit_gpio_direction_input:
...
static int bma2x2_remove(struct i2c_client *client)
{
	...
	bma2x2_set_enable(&client->dev, OFF);
