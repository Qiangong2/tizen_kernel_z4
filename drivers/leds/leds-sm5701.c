...
#define TORCH_BRIGHTNESS_LVL_1 6
#define TORCH_BRIGHTNESS_LVL_2 12
#define TORCH_BRIGHTNESS_LVL_3 18
#define TORCH_BRIGHTNESS_LVL_4 24
#define TORCH_BRIGHTNESS_LVL_5 31
...
static int sm5701_map_brightness(int brightness)
{
	int ret;

	if (brightness <= TORCH_BRIGHTNESS_LVL_1)
		ret = 0x02; //30mA current
	else if (brightness <= TORCH_BRIGHTNESS_LVL_2)
		ret = 0x04; //50mA current
	else if (brightness <= TORCH_BRIGHTNESS_LVL_3)
		ret = 0x07; //80mA current
	else if (brightness <= TORCH_BRIGHTNESS_LVL_4)
		ret = 0x0B; //120mA current
	else
		ret = 0x0F;  //160mA current

	return ret;
}

static void sm5701_torch_enable(int brightness)
{
	brightness = sm5701_map_brightness(brightness);
...
static void sm5701_torch_disable(int brightness)
{
	sm5701_led_ready(LED_DISABLE);
	sm5701_set_fleden(SM5701_FLEDEN_DISABLED);
	SM5701_operation_mode_function_control();
	/*
	 * Before turning off the torch, set default value so that next time when
	 * torch will turn on, it will turn on at default value
	 */
	sm5701_set_imled(brightness);
}
...
static int leds_sm5701_probe(struct platform_device *pdev)
{
	...
	torch_data.torch_enable = sm5701_torch_enable;
	torch_data.torch_disable = sm5701_torch_disable;
