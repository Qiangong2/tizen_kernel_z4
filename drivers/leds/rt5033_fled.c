...
#define GPIO_FRONT_FLASH_TORCH	156
#define GPIO_FRONT_FLASH_EN	54
...
static rt5033_fled_platform_data_t rt5033_default_fled_pdata = {
	...
	.fled_torch_current = RT5033_TORCH_CURRENT(65),
...
static int rt5033_fled_probe(struct platform_device *pdev)
{
	...
	ret = gpio_request(GPIO_FRONT_FLASH_TORCH, "GPIO_FRONT_FLASH_TORCH");
	if (ret) {
		dev_err(&pdev->dev,
			"Failed to request GPIO_FRONT_FLASH_TORCH\n");
	} else {
		ret = gpio_direction_output(GPIO_FRONT_FLASH_TORCH, 0);
		if (ret)
			dev_err(&pdev->dev,
				"%s : GPIO(%d) set direction failed!!\n",
				 __func__, GPIO_FRONT_FLASH_TORCH);
			gpio_free(GPIO_FRONT_FLASH_TORCH);
	}

	ret = gpio_request(GPIO_FRONT_FLASH_EN, "GPIO_FRONT_FLASH_EN");
	if (ret) {
		dev_err(&pdev->dev,
			"Failed to request GPIO_FRONT_FLASH_EN\n");
	} else {
		ret = gpio_direction_output(GPIO_FRONT_FLASH_EN, 0);
		if (ret)
			dev_err(&pdev->dev,
				"%s : GPIO(%d) set direction failed!!\n",
				 __func__, GPIO_FRONT_FLASH_EN);
			gpio_free(GPIO_FRONT_FLASH_EN);
	}
