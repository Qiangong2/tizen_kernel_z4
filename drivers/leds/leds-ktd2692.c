 ...
 *
 * Copyright (C) 2016-2017 Sunggeun Yim <sunggeun.yim@samsung.com>
 *
...
#define TORCH_BRIGHTNESS_LVL_1 15
#define TORCH_BRIGHTNESS_LVL_2 32
#define TORCH_BRIGHTNESS_LVL_3 49
#define TORCH_BRIGHTNESS_LVL_4 65
#define TORCH_BRIGHTNESS_LVL_5 82
#define TORCH_BRIGHTNESS_LVL_6 100

static struct class *camera_class; /*sys/class/ktd2692rearflash*/
...
EXPORT_SYMBOL(ktd2692_set_torch_current);
void ktd2692_set_torch_current(int curr)
{
	if (global_ktd2692data)
		global_ktd2692data->movie_current_value =
			KTD2692_MOVIE_CURRENT(curr, KTD2692_MAX_CURRENT);
}

static int ktd2692_map_torch_brightness(int brightness)
{
	int ret;
	...
	if (brightness <= TORCH_BRIGHTNESS_LVL_1)
		...
		ret = 104; //104mA current
	else
		ret = 156;  //156mA current

	return ret;
}

static void ktd2692_torch_enable(int brightness)
{
	brightness = ktd2692_map_torch_brightness(brightness);
	ktd2692_set_torch_current(brightness);
	ktd2692_led_mode_ctrl(3);
}

static void ktd2692_torch_disable(int brightness)
{
	ktd2692_led_mode_ctrl(1);
}

EXPORT_SYMBOL(ktd2692_set_flash_current);
void ktd2692_set_flash_current(int curr)
{
	if (global_ktd2692data)
		global_ktd2692data->flash_current_value =
			KTD2692_FLASH_CURRENT(curr, KTD2692_MAX_CURRENT);
}


void set_gpio_direction(int val)
{
	int ret = 1;
	ret = gpio_direction_output(global_ktd2692data->flash_control, val);
	if (ret) {
...
static int ktd2692_parse_dt(struct device *dev, struct ktd2692_platform_data *pdata)
{
...
static int ktd2692_probe(struct platform_device *pdev)
{
	...
	camera_class = class_create(THIS_MODULE, "ktd2692rearflash");
	if (IS_ERR(camera_class)) {
		LED_ERROR("Failed to create class rearflash!\n");
	...
	ret = gpio_request(global_ktd2692data->flash_control,
						"ktd2692_led_control");
	if (ret) {
		LED_ERROR("Failed to request ktd2692_led_mode_ctrl\n");
	} else {
		ret = gpio_direction_output(
				global_ktd2692data->flash_control, 0);
		if (ret)
			LED_ERROR("%s gpio_direction_output %d failed!!\n",
								__func__, 0);
		gpio_free(global_ktd2692data->flash_control);
	}

	torch_data.torch_enable = ktd2692_torch_enable;
	torch_data.torch_disable = ktd2692_torch_disable;
