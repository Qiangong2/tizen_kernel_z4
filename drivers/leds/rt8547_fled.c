...
#define TORCH_BRIGHTNESS_LVL_1 3
#define TORCH_BRIGHTNESS_LVL_2 6
#define TORCH_BRIGHTNESS_LVL_3 9
#define TORCH_BRIGHTNESS_LVL_4 12
#define TORCH_BRIGHTNESS_LVL_5 15
...
static int rt8547_map_brightness(int brightness)
{
	int ret;

	if (brightness <= TORCH_BRIGHTNESS_LVL_1)
		ret = 0x00; //25mA current
	else if (brightness <= TORCH_BRIGHTNESS_LVL_2)
		ret = 0x01; //50mA current
	else if (brightness <= TORCH_BRIGHTNESS_LVL_3)
		ret = 0x02; //75mA current
	else if (brightness <= TORCH_BRIGHTNESS_LVL_4)
		ret = 0x05; //150mA current
	else if (brightness <= TORCH_BRIGHTNESS_LVL_5)
		ret = 0x08; //225mA current
	else
		ret = 0x08;  //225mA current
...
static void rt8547_torch_enable(int brightness)
{
	brightness = rt8547_map_brightness(brightness);
	rt8547_control(1, brightness);
}

static void rt8547_torch_disable(int brightness)
{
	rt8547_control(0, brightness);
}
...
static void rt8547_reset(rt8547_fled_info_t *info)
{
	...
	__rt8547_send_data(info, RT8547_FLED_CONTROL3,
			RT8547_FLED_SW_RESET_MASK);
}
...
{
	if (torch)
		rt8547_set_bits(&info->base, RT8547_FLED_CONTROL3,
				RT8547_FLED_MODE_MASK);
	else
		rt8547_clr_bits(&info->base, RT8547_FLED_CONTROL3,
				RT8547_FLED_MODE_MASK);
}

static int rt8547_fled_set_mode(struct rt_fled_info *fled_info,
...
static int rt8547_gpio_init(const rt8547_fled_platform_data_t *pdata)
{
	...
	ret = gpio_request_one(pdata->flen_gpio, GPIOF_OUT_INIT_LOW,
				"rt8547_flen");
	if (ret < 0) {
	...
	if (pdata->flen_gpio != pdata->ctl_gpio) {
		ret = gpio_request_one(pdata->ctl_gpio,
				GPIOF_OUT_INIT_LOW, "rt8547_ctl");
		if (ret < 0) {
	...
	ret = gpio_request_one(pdata->flset_gpio, GPIOF_OUT_INIT_LOW,
				"rt8547_flset");
	if (ret < 0) {
...
static int rt8547_fled_suspend(struct rt_fled_info *fled_info,
		pm_message_t state)
{
	//rt8547_fled_info_t *info = (rt8547_fled_info_t *)fled_info;
...
/* otherwise current in mA*/
static int rt8547_fled_troch_current_list(struct rt_fled_info *info,
		int selector)
{
...
{
	int ret;
	struct platform_device *pdev = container_of(&(rt_fled_pdev.dev.parent),
			struct platform_device, dev);
	rt8547_fled_info_t *fled_info = platform_get_drvdata(pdev);

	RT8547_WARN("flash_mode:0x%x,level:0x%x	line %d\n",
			mode, level, __LINE__);
	switch (mode) {
	case FLASHLIGHT_MODE_OFF:
		...
		struct device_attribute *attr, const char *buf, size_t c)
{
	...
	RT8547_WARN("rt8547_fled_store  buf=%s,flash_mode:%x line %d .\n",
			buf, flash_mode, __LINE__);
	rt8547_control(flash_mode >> 6, flash_mode & 0x3f);
	return c;
}
static DEVICE_ATTR(flash, 0644, show_help, rt8547_fled_store);
...
static int rt8547_parse_dt(struct device_node *np, struct device *dev,
		rt8547_fled_platform_data_t *pdata)
{
	int ret;
	...
	ret = of_property_read_u32(np, "fled,strobe_current",
			&pdata->fled_strobe_current);
	pdata->fled_strobe_current = RT8547_STROBE_CURRENT(
			pdata->fled_strobe_current);
	if (ret < 0)
		return ret;

	ret = of_property_read_u32(np, "fled,timeout_current_level",
			&pdata->fled_timeout_current_level);
	pdata->fled_timeout_current_level = RT8547_TIMEOUT_LEVEL(
			pdata->fled_timeout_current_level);
	if (ret < 0)
		return ret;

	ret = of_property_read_u32(np, "fled,torch_current",
			&pdata->fled_torch_current);
	pdata->fled_torch_current = RT8547_TORCH_CURRENT(
			pdata->fled_torch_current);
	if (ret < 0)
		return ret;

	ret = of_property_read_u32(np, "fled,strobe_timeout",
			&pdata->fled_strobe_timeout);
	pdata->fled_strobe_timeout = RT8547_STROBE_TIMEOUT(
			pdata->fled_strobe_timeout);
	if (ret < 0)
		return ret;

	ret = of_property_read_u32(np, "fled,lv_protection",
			&pdata->fled_lv_protection);
	pdata->fled_lv_protection = RT8547_LV_PROTECTION(
			pdata->fled_lv_protection);
	RT8547_INFO("ethanhu add %d,%d,%d,%d,%d,%d,%d", pdata->flen_gpio,
		pdata->flset_gpio, pdata->fled_strobe_current,
		pdata->fled_timeout_current_level, pdata->fled_torch_current,
...
static int rt8547_fled_probe(struct platform_device *pdev)
{
	...
	struct torch_sec_data torch_data;
	...
	if (ret < 0) {
		RT8547_WARN("Failed to create attribute val.");
	...
	torch_data.torch_enable = rt8547_torch_enable;
	torch_data.torch_disable = rt8547_torch_disable;
	torch_sec_register_flash_led_data(torch_data);
...
static int __rt8547_fled_suspend(struct platform_device *pdev,
		pm_message_t state)
{
