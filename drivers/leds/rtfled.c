...
#define RT_FLED_REAR_DEVICE  "rt-flash-rear-led"
...
static int rtfled_probe(struct platform_device *pdev)
{
	...
	info->flashlight_dev = flashlight_device_register(
				       pdev->name ? pdev->name : RT_FLED_DEVICE,
...
static struct platform_device_id rt_leds_ids[] = {
	{
		.name		= RT_FLED_DEVICE,
	},
	{
		.name		= RT_FLED_REAR_DEVICE,
	},
};
