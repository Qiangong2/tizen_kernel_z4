...
static unsigned long sprd_thm_temp_read(struct sprd_thermal_zone *pzone)
{
	...
	pr_debug("sensor id:%d, rawdata:0x%x, temp:%lu\n", sensor, rawdata, temp);
	return temp;
...
static int sprd_thm_hw_init(struct sprd_thermal_zone *pzone)
{
	...
	pr_info("sprd_thm_hw_init 2713s_thm id:%d, base 0x%lx\n",
...
#endif
	pr_info("sprd_thm_hw_init addr 0x:%lx, int ctrl 0x%x\n",
			local_sensor_addr,
                         ...
                         struct device *dev)
{
	...
	pr_debug("the trip_num=%d, sensor_id =%d\n", pdata->num_trips, pdata->sensor_id);
	pdata++;
	};
	return thm_data;
...
static int sprd_ddie_thm_probe(struct platform_device *pdev)
{
...
#endif
	pr_debug("sprd_ddie_thm_probe start\n");
	...
	pr_debug("the thermal-name1 is %s\n", pzone->thermal_zone_name);
	...
	pr_debug("sprd_ddie_thm_probe---end\n");
