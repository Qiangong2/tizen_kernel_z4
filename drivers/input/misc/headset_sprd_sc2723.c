...
static int adc_get_average(int gpio_num, int gpio_value,int scale)
{
    int i = 0;
    int j = 0;
    int k = 0;
    ...
    struct sprd_headset *ht = &headset;
    struct sprd_headset_platform_data *pdata = ht->platform_data;
    PRINT_DBG("ADC_CHANNEL_HEADMIC:0x%x scale:%d", ADC_CHANNEL_HEADMIC, scale);
    //================= debug =====================
    if(debug_level >= 3) {
        int count = 0;
        int adc_val = 0;
        ...
        while(count < 20) {
            count++;
            adc_val = wrap_sci_adc_get_value(ADC_CHANNEL_HEADMIC, scale);
	...
	headset_reg_clr_bit(HEADMIC_DETECT_REG(ANA_PMU0),
				AUDIO_HEADMIC_SLEEP_EN);
	PRINT_INFO("ANA_PMU0 = %#X\n",
			sci_adi_read(HEADMIC_DETECT_REG(ANA_PMU0)));
    }
    ...
    for(i=0; i< ADC_READ_LOOP; i++) {
        ...
        adc[0] = wrap_sci_adc_get_value(ADC_CHANNEL_HEADMIC, scale);
        ...
        for(j=0; j<ADC_READ_COUNT-1; j++) {
            ...
            adc[j+1] = wrap_sci_adc_get_value(ADC_CHANNEL_HEADMIC, scale);
...
static int headset_plug_confirm_by_adc(int last_gpio_detect_value,int scale)
{
    struct sprd_headset *ht = &headset;
    struct sprd_headset_platform_data *pdata = ht->platform_data;
    int adc_last = 0;
    ...
    adc_last = adc_get_average(pdata->gpio_detect, last_gpio_detect_value,scale);
    if(-1 == adc_last) {
    ...
    while(count < PLUG_CONFIRM_COUNT) {
            ...
            adc_current = adc_get_average(pdata->gpio_detect, last_gpio_detect_value,scale);
            if(-1 == adc_current) {
                    PRINT_INFO("headset_plug_confirm_by_adc failed!!!\n");
                    return -1;
            }
...
no_mic_retry:
    ...
    if(EIC_AUD_HEAD_INST2 == pdata->gpio_detect) {
        ...
        msleep(50); //100
        adc_left_average = headset_plug_confirm_by_adc(last_gpio_detect_value, 0);
    ...
    msleep(50);
#endif
    adc_mic_average = headset_plug_confirm_by_adc(last_gpio_detect_value, 0);
...
static void headset_button_work_func(struct work_struct *work)
{
    ...
    if(1 == button_state_current) {//pressed!
        if(ht->platform_data->nbuttons > 0) {
                adc_mic_average = adc_get_average(pdata->gpio_button, gpio_button_value_last,0);
                if(-1 == adc_mic_average) {
