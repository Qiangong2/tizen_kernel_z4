...
static cmd_t g_cmd_table[] =
{
	...
	CMD_ITEM(WIFI_CMD_DISASSOC),
	CMD_ITEM(WIFI_CMD_SDIO_CHN_FLUSH),
	CMD_ITEM(WIFI_CMD_COMMON_DATA),
...
int wlan_cmd_send_to_ic(unsigned char vif_id, unsigned char *data, int len, int subtype)
{
	int ret;
	tx_msg_t    msg    = {0};
	msg_q_t    *msg_q  = &(g_wlan.netif[vif_id].msg_q[MSG_Q_ID_4]);
	wlan_cmd_t *cmd    = &(g_wlan.cmd);
...
int wlan_cmd_enable_whitelist(unsigned char vif_id, u8 *mac_addr)
{
	...
	wlan_cmd_send_recv(vif_id, (unsigned char *)mac_addr, 6 * mac_addr[0] + 1,
				WIFI_CMD_ENABLE_WHITELIST, CMD_WAIT_TIMEOUT);
...
int wlan_cmd_disable_whitelist(unsigned char vif_id, u8 *mac_addr)
{
	...
	wlan_cmd_send_recv(vif_id, (unsigned char *)mac_addr, 6 * mac_addr[0] + 1,
				WIFI_CMD_DISABLE_WHITELIST, CMD_WAIT_TIMEOUT);
...
int wlan_cmd_set_bssid(unsigned char vif_id, const unsigned char *addr)
{
	...
	hex_dump("sme->bssid is", strlen("sme->bssid is"), addr, 6);
...
{
#ifdef CONFIG_TIZEN
  /* Make sure to use TIZEN path whenever SPRD releases driver code */
...
int wlan_rx_rsp_process(const unsigned char vif_id, r_msg_hdr_t *msg)
{
	...
	if (mutex_trylock(&cmd->mem_lock)) {
		...
		if (msg->len + sizeof(r_msg_hdr_t) > WLAN_CMD_MEM_LEN) {
			printke("[RECV_RSP] malformed pkt!\n");
