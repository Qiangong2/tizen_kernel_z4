...
if [ "${RELEASE}" = "usr" ]; then
	...
	echo "Now disable CONFIG_SECURITY_PROBES for ${MODEL}_defconfig"

	sed -i 's/CONFIG_SECURITY_PROBES=y/\# CONFIG_SECURITY_PROBES is not set/g' arch/${ARM}/configs/${MODEL}_defconfig
	if [ "$?" != "0" ]; then
		echo "Failed to disable CONFIG_DYNAMIC_DEBUG feature"
		exit 1
	fi

	echo "Now disable CONFIG_AUDITSYSCALL for ${MODEL}_defconfig"

	sed -i 's/CONFIG_AUDITSYSCALL=y/\# CONFIG_AUDITSYSCALL is not set/g' arch/${ARM}/configs/${MODEL}_defconfig
	if [ "$?" != "0" ]; then
		echo "Failed to disable CONFIG_AUDITSYSCALL feature"
		exit 1
	fi
fi
